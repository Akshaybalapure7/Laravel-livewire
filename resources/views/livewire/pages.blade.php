                            ...
                            @if ($data->count())
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        {{ $item['title'] }}
                                         {!! $item['is_dafault_home']==1 ?'<span class="text-green-400 text-xs font-bold">[Default Home Page]</span>':'' !!}
                                         {!! $item['is_default_not_found']== 1 ? '<span style="color:red;" class="text-red-400 text-xs font-bold">[Default 404 Page]</span>':'' !!}
                                        </td>
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <label for="">
                    <input type="checkbox"
                     value="{{ $isSetToDefaultHomePage }}"
                      wire:model='isSetToDefaultHomePage'
                      class="form-checkbox"
                     />
                     <span class="ml-2 text-sm text-gray-800"> Set as default home page</span>  
                </label>
            </div>
            <div class="mt-4">
                <label for="">
                    <input type="checkbox"
                     value="{{ $isSetToDefaultNotFoundPage }}"
                      wire:model='isSetToDefaultNotFoundPage'
                      class="form-checkbox"
                     />
                     <span class="ml-2 text-sm text-red-800" style="color:red;"> Set as default error 404 page</span>  
                </label>
            </div>
            <div class="mt-4">
                <x-jet-label for='title' vlaue="{{ __('Content') }}" />
